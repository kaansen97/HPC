...
class Field:
   ...
   def exchange_startall(self):
       ...
       # ... implement ...
       self._req = [MPI.REQUEST_NULL] * 4
       if domain.neighbour_north >= 0:
           self._req[0] = domain.comm.Irecv([self._bdryN, MPI.DOUBLE], source=domain.neighbour_north)
           self._buffN[:] = self._inner[:, domain.local_ny - 1]
           domain.comm.Isend([self._buffN, MPI.DOUBLE], domain.neighbour_north)
 
       if domain.neighbour_south >= 0:
           self._req[1] = domain.comm.Irecv([self._bdryS, MPI.DOUBLE], source=domain.neighbour_south)
           self._buffS[:] = self._inner[:, 0]
           domain.comm.Isend([self._buffS, MPI.DOUBLE], domain.neighbour_south)
 
       if domain.neighbour_east >= 0:
           self._req[2] = domain.comm.Irecv([self._bdryE, MPI.DOUBLE], source=domain.neighbour_east)
           self._buffE[:] = self._inner[domain.local_nx-1, :]
           domain.comm.Isend([self._buffE, MPI.DOUBLE], domain.neighbour_east)
 
 
       if domain.neighbour_west >= 0:
           self._req[3] = domain.comm.Irecv([self._bdryW, MPI.DOUBLE], source=domain.neighbour_west)
           self._buffW[:] = self._inner[0, :]
           domain.comm.Isend([self._buffW, MPI.DOUBLE], domain.neighbour_west)
