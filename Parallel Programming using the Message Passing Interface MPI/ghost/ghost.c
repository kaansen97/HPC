...
int main(int argc, char *argv[])
{
    ...
    if (size!=16) {
        printf("please run this with 8 processors\n");
        MPI_Finalize();
        exit(1);
    }
    ...
    MPI_Type_vector(6, 1, 1, MPI_DOUBLE, &data_ghost_row);
    MPI_Type_commit(&data_ghost_row);
    // data_ghost (col)
    MPI_Datatype data_ghost_col;
    MPI_Type_vector(6, 1, 8, MPI_DOUBLE, &data_ghost_col);
    MPI_Type_commit(&data_ghost_col);


    //  to the top
    
    MPI_Irecv(&data[DOMAINSIZE*DOMAINSIZE-DOMAINSIZE+1], 1, data_ghost_row, rank_bottom, 0, comm_cart, &request);
    MPI_Send(&data[DOMAINSIZE+1], 1, data_ghost_row, rank_top, 0, comm_cart);
    MPI_Wait(&request,&status);

    //  to the bottom
    MPI_Irecv(&data[1], 1, data_ghost_row, rank_top, 1, comm_cart, &request);

    MPI_Send(&data[(DOMAINSIZE * DOMAINSIZE)-(2*DOMAINSIZE)+1], 1, data_ghost_row, rank_bottom, 1, comm_cart);
    // Wait for the non-blocking communications to complete
    MPI_Wait(&request,&status);

    //  to the left
    MPI_Irecv(&data[2*DOMAINSIZE-1], 1, data_ghost_col, rank_right, 2, comm_cart, &request);
    MPI_Send(&data[DOMAINSIZE+1], 1, data_ghost_col, rank_left, 2, comm_cart);
    MPI_Wait(&request,&status);

    //  to the right
    MPI_Irecv(&data[DOMAINSIZE], 1, data_ghost_col, rank_left, 2, comm_cart, &request);
    MPI_Send(&data[DOMAINSIZE+6], 1, data_ghost_col, rank_right, 2, comm_cart);
    // Wait for the non-blocking communications to complete
    MPI_Wait(&request,&status);
