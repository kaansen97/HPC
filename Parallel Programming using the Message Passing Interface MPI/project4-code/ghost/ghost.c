...
int main(int argc, char *argv[])
{
    ...
    // data_row
    MPI_Datatype data_row;
    MPI_Type_vector(6, 1, 1, MPI_DOUBLE, &data_row);
    MPI_Type_commit(&data_row);
    // data_ghost (col)
    ...
    //  to the top
    MPI_Isend(&data[DOMAINSIZE], 1, data_ghost, rank_top, 0, comm_cart, &request[0]);
    MPI_Irecv(&data[0], 1, data_ghost, rank_top, 0, comm_cart, &request[1]);

    //  to the bottom
    MPI_Isend(&data[DOMAINSIZE * (SUBDOMAIN + 1)], 1, data_ghost, rank_bottom, 0, comm_cart, &request[2]);
    MPI_Irecv(&data[DOMAINSIZE * (SUBDOMAIN + 2)], 1, data_ghost, rank_bottom, 0, comm_cart, &request[3]);

    //  to the left
    MPI_Isend(&data[1], 1, data_ghost, rank_left, 0, comm_cart, &request[4]);
    MPI_Irecv(&data[0], 1, data_ghost, rank_left, 0, comm_cart, &request[5]);

    //  to the right
    MPI_Isend(&data[DOMAINSIZE + SUBDOMAIN], 1, data_ghost, rank_right, 0, comm_cart, &request[6]);
    MPI_Irecv(&data[DOMAINSIZE + SUBDOMAIN + 1], 1, data_ghost, rank_right, 0, comm_cart, &request[7]);
